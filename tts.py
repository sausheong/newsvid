...
Examples:
    ...
    kokoro-tts input.txt output.wav --background music.mp3
    kokoro-tts script_with_pauses.txt output.wav --speed 1.2
    """)
...
def convert_text_to_audio(input_file, output_file=None, voice=None, speed=1.0, lang="en-us", 
                         stream=False, split_output=None, format="wav", debug=False, background_file=None):
    global stop_spinner
    # Load Kokoro model
    try:
        kokoro = Kokoro("kokoro-v1.0.onnx", "voices-v1.0.bin")
    ...
    if input_file.endswith('.epub'):
        ...
        # Check if this is a script with pauses
        if "PAUSE_" in text:
            ...
            try:
                # Parse the script to extract text chunks and pause times
                text_chunks, pause_times = parse_script_with_pauses(text)
                
                if debug:
                    print(f"Found {len(text_chunks)} text chunks and {len(pause_times)} pauses")
                    print(f"Pause times: {pause_times}")
                
                # Process each text chunk
                print(f"Processing {len(text_chunks)} chunks with pauses...")
                for i, chunk in enumerate(text_chunks):
                    chunk_file = os.path.join(temp_dir, f"chunk_{i:03d}.{format}")
                    ...
                    spinner_thread = threading.Thread(
                        ...
                        args=(f"Processing chunk {i+1}/{len(text_chunks)}",)
                    )
                    ...
                    try:
                        # Process the chunk
                        samples, sample_rate = process_chunk_sequential(chunk, kokoro, voice, speed, lang, retry_count=0, debug=debug)
                        if samples is not None:
                ...
                # Combine chunks with pauses
                if not output_file:
                    output_file = f"{os.path.splitext(input_file)[0]}.{format}"
                
                combine_audio_with_pauses(chunk_files, pause_times, output_file, background_file)
                print(f"Created {output_file} with pauses")
                
            finally:
                # Clean up temporary files
                shutil.rmtree(temp_dir)
            
            # Since we've handled the audio generation, we can return here
            return
...
def parse_script_with_pauses(text):
    """Parse text content and extract pause durations."""
    lines = text.splitlines()
    process = [line for line in lines if line]
    pause_times = []
    text_chunks = []
    ...
    for line in process:
        if "PAUSE_" in line:
            try:
                pause_times.append(float(line.split("PAUSE_")[1]))
            except (ValueError, IndexError):
    ...
    return text_chunks, pause_times

def combine_audio_with_pauses(audio_chunks, pause_times, output_file, background_file=None):
    """Combine audio chunks with specified pauses between them."""
    final_audio = AudioSegment.empty()
    
    for idx, audio_chunk in enumerate(audio_chunks):
        chunk = AudioSegment.from_file(audio_chunk)
        final_audio += chunk
        
        if idx < len(pause_times):
            silence = AudioSegment.silent(duration=pause_times[idx] * 1000)
            final_audio += silence
    ...
    if background_file:
        try:
            ...
            # Trim background to match speech length plus fade out
            fade_out_duration = 2000  # 2 seconds
            trimmed_background = background[:len(final_audio) + fade_out_duration]
            trimmed_background = trimmed_background.fade_out(fade_out_duration)
            
            # Lower background volume
            background_vol = 8  # dB reduction
            trimmed_background = trimmed_background - background_vol
            
            # Overlay speech on background
            final_audio = trimmed_background.overlay(final_audio)
            
        except Exception as e:
            print(f"Error processing background audio: {e}")
...
def get_valid_options():
    ...
    return {
        ...
        '--format',
        '--debug',
        '--background'  # Add background option
    }
...
if __name__ == "__main__":
    ...
    while i < len(sys.argv):
        ...
        if arg.startswith('--') or arg.startswith('-'):
            ...
            elif arg in {'--speed', '--lang', '--voice', '--split-output', '--format', '--background'}:
                i += 1
        i += 1
    ...
    # Get background file if specified
    background_file = None
    for i in range(1, len(sys.argv) - 1):
        if sys.argv[i] == '--background':
            background_file = sys.argv[i + 1]
            break
    ...
    convert_text_to_audio(input_file, output_file, voice=voice, stream=stream, 
                         ...
                         format=format, debug=debug, background_file=background_file)
